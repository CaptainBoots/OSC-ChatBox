...
void MainWindow::buildRoot() {
    setWindowTitle("SynthTool-Launcher");
    resize(580, 600);
    ...
    QLabel* titleLbl = new QLabel("◈ SynthTool-Launcher");
...
void MainWindow::checkForUpdates() {
    // Check main update from GitHub raw URL of ProtoTool-Launcher
    QString branch = ConfigManager::instance().updateBranch();
    QString url = "https://raw.githubusercontent.com/CaptainBoots/ProtoTool-Launcher/" + branch + "/ProtoTool-Launcher.py";
            ...
            if (match.hasMatch()) {
                ...
                if (newer) {
                    ...
                    auto btn = QMessageBox::question(this, "Update Available",
                        QString("A new version of SynthTool-Launcher (v%1) is available.\n\nWould you like to open the GitHub releases page to download it?")
                        .arg(remoteVer), QMessageBox::Yes | QMessageBox::No);
                    if (btn == QMessageBox::Yes) {
                        QDesktopServices::openUrl(QUrl("https://github.com/CaptainBoots/ProtoTool-Launcher/releases"));
                    }
                } else {
...
void MainWindow::fetchRepoTree() {
    ...
    QNetworkRequest request((QUrl(url)));
    request.setHeader(QNetworkRequest::UserAgentHeader, "SynthTool-Launcher-Cpp");
