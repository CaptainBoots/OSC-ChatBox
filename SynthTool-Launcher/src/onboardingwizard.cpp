...
OnboardingWizard::OnboardingWizard(QWidget* parent) : QDialog(parent) {
    setWindowTitle("SynthTool-Launcher Setup Wizard");
    ...
    // Let's force it to end with SynthTool-Launcher
    if (!m_toolsDir.endsWith("SynthTool-Launcher", Qt::CaseInsensitive)) {
        m_toolsDir = QDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)).filePath("SynthTool-Launcher");
    }
    ...
    QLabel* p1Title = new QLabel("Welcome to SynthTool-Launcher! ✨");
    ...
    QLabel* p1Desc = new QLabel(
        ...
        "SynthTool-Launcher lets you easily download, run, and update all of your "
