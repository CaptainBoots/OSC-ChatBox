...
QString ConfigManager::centralConfigDir() const {
    ...
    if (!path.endsWith("CToolBox-Launcher", Qt::CaseInsensitive)) {
        path = QDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)).filePath("CToolBox-Launcher");
    }
    QDir().mkpath(path);
