...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QFrame, QMessageBox, QLineEdit, QFileDialog,
    QComboBox,
)
...
def open_settings(parent, cfg: dict, save_cb, reset_cb, theme_cb,
                   account_status_cb=None, login_cb=None, logout_cb=None, import_cb=None):
    ...
    §8.6 rather than a parallel per-tool copy. import_cb, if given,
    re-runs the official-VRChat-favorites import flow on demand."""
    dlg = QDialog(parent)
    ...
    # ── Favorites Folder ────────────────────────────────────────────────
    if "favorites_dir" in cfg:
        _section_label(inner_layout, "Favorites Folder")

        dir_note = QLabel(
            "Each group is a plain JSON file in a subfolder here — safe "
            "to back up, sync, or hand-edit."
        )
        dir_note.setWordWrap(True)
        dir_note.setStyleSheet(f"color: {theme.SUBTEXT}; background: transparent; border: none;")
        dir_note.setFont(theme.qt_font(7))
        inner_layout.addWidget(dir_note)

        dir_row = QHBoxLayout()
        dir_edit = QLineEdit(cfg.get("favorites_dir", ""))
        dir_edit.setStyleSheet(theme.line_edit_qss())
        dir_edit.setFont(theme.qt_font(9))

        def _on_dir_changed():
            cfg["favorites_dir"] = dir_edit.text().strip() or cfg["favorites_dir"]
            save_cb()
        ...
        def _browse():
            chosen = QFileDialog.getExistingDirectory(dlg, "Choose Favorites Folder", dir_edit.text())
            if chosen:
        ...
        restart_note = QLabel("Changing this takes effect next launch.")
        restart_note.setStyleSheet(f"color: {theme.SUBTEXT}; background: transparent; border: none;")
        restart_note.setFont(theme.qt_font(7))
        inner_layout.addWidget(restart_note)

        if import_cb is not None:
            import_row = QHBoxLayout()
            import_row.addStretch(1)
            import_btn = QPushButton("Import from VRChat Favorites...")
            import_btn.setStyleSheet(theme.subtle_button_qss())
            import_btn.setFont(theme.qt_font(9))
            import_btn.clicked.connect(import_cb)
            import_row.addWidget(import_btn)
            import_row.addStretch(1)
            inner_layout.addLayout(import_row)
